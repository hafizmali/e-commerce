...
class acf_field_gallery extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'gallery';
		$this->label = __("Gallery",'ecoshop');
		$this->category = __("Content",'ecoshop');
...
acf.fields.gallery.title_add = "<?php _e("Add Image to Gallery",'ecoshop'); ?>";
acf.fields.gallery.title_edit = "<?php _e("Edit Image",'ecoshop'); ?>";
</script>
<?php
	}
...
<div class="acf-gallery" data-preview_size="<?php echo ''.$field['preview_size']; ?>">
	...
	<div class="thumbnails">
		...
		<?php if( $field['value'] ): foreach( $field['value'] as $attachment ): 
			...
			<div class="thumbnail" data-id="<?php echo ''.$attachment['id']; ?>">
				...
				<div class="inner clearfix">
					...
					<div class="list-data">
						<table>
							<tbody>
							<tr>
								<th><label><?php _e("Title",'ecoshop'); ?>:</label></th>
								<td class="td-title"><?php echo ''.$attachment['title']; ?></td>
							</tr>
							<tr>
								<th><label><?php _e("Alternate Text",'ecoshop'); ?>:</label></th>
								<td class="td-alt"><?php echo ''.$attachment['alt']; ?></td>
							</tr>
							<tr>
								<th><label><?php _e("Caption",'ecoshop'); ?>:</label></th>
								<td class="td-caption"><?php echo ''.$attachment['caption']; ?></td>
							</tr>
							<tr>
								<th><label><?php _e("Description",'ecoshop'); ?>:</label></th>
				...
				<div class="hover">
					<ul class="bl">
						<li><a href="#" class="acf-button-delete ir"><?php _e("Remove",'ecoshop'); ?></a></li>
						<li><a href="#" class="acf-button-edit ir"><?php _e("Edit",'ecoshop'); ?></a></li>
					</ul>
				</div>
	...
	<div class="toolbar">
		<ul class="hl clearfix">
			<li class="add-image-li"><a class="acf-button add-image" href="#"><?php _e("Add Image",'ecoshop'); ?></a></li>
			<li class="gallery-li view-grid-li active"><div class="divider divider-left"></div><a class="ir view-grid" href="#"><?php _e("Grid",'ecoshop'); ?></a><div class="divider"></div></li>
			<li class="gallery-li view-list-li"><a class="ir view-list" href="#"><?php _e("List",'ecoshop'); ?></a><div class="divider"></div></li>
			<li class="gallery-li count-li right">
				<span class="count" data-0="<?php _e("No images selected",'ecoshop'); ?>" data-1="<?php _e("1 image selected",'ecoshop'); ?>" data-2="<?php _e("{count} images selected",'ecoshop'); ?>"></span>
			</li>
		</ul>
	</div>
...
<tr class="field_option field_option_<?php echo ''.$this->name; ?>">
	<td class="label">
		<label><?php _e("Preview Size",'ecoshop'); ?></label>
		<p class="description"><?php _e("Thumbnail is advised",'ecoshop'); ?></p>
	</td>
	<td>
		<?php
...
(function($){
	...
	// add message
	self.parent.acf.helpers.add_message('<?php _e("Image Updated",'ecoshop'); ?>.', div);
...
(function($){
	...
	*/
	...
	function disable_image( div )
	{
		...
		// change button text
		div.find('.acf-select').addClass('disabled').text("<?php _e("Added",'ecoshop'); ?>");
	...
	*/
	...
	$('#media-items .media-item a.acf-select').live('click', function(){
		...
		{
			alert("<?php _e("Image already exists in gallery",'ecoshop'); ?>");
			return false;
		}
	...
	function acf_add_buttons()
	{
		...
		$('#media-items .media-item:not(.acf-active)').each(function(){
			...
			// Add select button
			$(this).find('.filename.new').before('<a href="' + id + '" class="button acf-select"><?php _e("Add Image",'ecoshop'); ?></a>');
			
			// add save changes button
			$(this).find('tr.submit input.button').hide().before('<input type="submit" value="<?php _e("Update Image",'ecoshop'); ?>" class="button savebutton" />');

...
function road_woo_price_html( $price, $product ){
	...
	if($product->get_type()=="variable") {
		if($product->get_variation_sale_price() && $product->get_variation_regular_price()!=$product->get_variation_sale_price()){
			...
			return '<span class="old-price">'. ( ( is_numeric( $rprice ) ) ? wc_price( $rprice ) : $rprice ) .'</span><span class="special-price">'.( ( is_numeric( $sprice ) ) ? wc_price( $sprice ) : $sprice ) .'</span>';
		} else {
			$rprice = $product->get_variation_regular_price();
			return '<span class="special-price">' . ( ( is_numeric( $rprice ) ) ? wc_price( $rprice ) : $rprice ) . '</span>';
		}
	}
    if ( $product->get_price() > 0 ) {
        ...
        $sprice = $product->get_price();
        return '<span class="special-price">' . ( ( is_numeric( $sprice ) ) ? wc_price( $sprice ) : $sprice ) . '</span>';
		}
	} else {
...
//move the breadcrumbs bar
remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20 );
add_action( 'woocommerce_show_breadcrumbs', 'woocommerce_breadcrumb', 20 );
...
function road_product_stock_status(){
	...
	<div class="stock-status">
		<label><?php _e('Availability', 'roadthemes');?></label>:&nbsp;
		<?php if($product->is_in_stock()){ ?>
			<strong><?php _e('In stock', 'roadthemes');?></strong>
		<?php } else { ?>
			<strong><?php _e('Out of stock', 'roadthemes');?></strong>
		<?php } ?>
	</div>
	<?php
}
add_action( 'woocommerce_single_product_summary', 'road_product_stock_status', 5 );

//Show countdown on product page
function road_product_sharefriend(){
	global $product;
...
add_action( 'woocommerce_single_product_summary', 'road_product_sharefriend', 30 );
...
function road_setup() {
	...
	add_image_size( 'category-thumb', 670, 402, true ); // (cropped)
	...
	*/
	...
	if($road_opt['enable_less']){
		...
		switch ($presetopt) {
			...
			case 2:
				$themevariables['primary_color'] = '#f65d3f';
				$themevariables['sale_color'] = '#f65d3f';
				$themevariables['saletext_color'] = '#fff';
				$themevariables['rate_color'] = '#363636';
			break;
				
			case 3:
				$themevariables['primary_color'] = '#e4bd9f';
				$themevariables['sale_color'] = '#ed6b0b';
				$themevariables['saletext_color'] = '#fff';
				$themevariables['rate_color'] = '#363636';
			break;
			
			case 4:
				$themevariables['primary_color'] = '#363636';
				$themevariables['sale_color'] = '#363636';
				$themevariables['saletext_color'] = '#fff';
				$themevariables['rate_color'] = '#363636';
			break;
		}
...
function road_widgets_init() {
	...
	register_sidebar( array(
		'name' => __( 'Blog Aside', 'roadthemes' ),
		'id' => 'blog-aside',
		'description' => __( 'Blog Aside', 'roadthemes' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title"><span>',
		'after_title' => '</span></h3>',
...
function road_entry_meta() {
	...
	if ( is_single() ) {
		$utility_text = __( '<i class="fa fa-user"></i>%1$s<i class="fa fa-clock-o"></i>%2$s<i class="fa fa-folder-o"></i>%3$s<i class="fa fa-tags"></i>%4$s%5$s', 'roadthemes' );
	} else {
		$utility_text = __( '<i class="fa fa-user"></i>%1$s<i class="fa fa-clock-o"></i>%2$s<i class="fa fa-folder-o"></i>%3$s%5$s', 'roadthemes' );
	}
	printf( $utility_text, $author, $date, $categories_list, $tag_list, $write_comments );
}
endif;
...
function road_entry_meta_small() {
	...
	$utility_text = __( '<div class="small-meta"><i class="fa fa-calendar"></i>%1$s%2$s</div>', 'roadthemes' );
